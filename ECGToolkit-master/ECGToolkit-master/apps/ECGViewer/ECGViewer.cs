				...
				lock (this)
				{
					...
					else
					{
						...
						{     
                            ...
                            var secondLead = sig.GetLeads()[1].Rhythm;

                            StringBuilder sb = new StringBuilder();

                            sb.Append(secondLead[0]);

                            for(int i = 1; i<secondLead.Length; i++)
                            {
                                sb.Append(Environment.NewLine);
                                sb.Append(secondLead[i]);
                            }

                            using (System.IO.StreamWriter file = new System.IO.StreamWriter("ecg.csv"))
                            {
